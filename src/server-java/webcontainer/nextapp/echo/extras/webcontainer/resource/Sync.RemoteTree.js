...
Extras.Sync.RemoteTree = Core.extend(Echo.Render.ComponentSync, {
    ...
    _getComponentByRenderId: function(renderId) {
        var c = this.component.application.getComponentByRenderId(renderId);
        if( c == undefined )
          c = null;
        return c;
    },
    ...
    renderAdd: function(update, parentElement) {
        ...
        this._renderNode(update, this.component.treeStructure.getRootNode());
    ...
    _renderNodeRecursive: function(update, node, iterator, depth, insertBefore, visible) {
        ...
        var com = this._getComponentByRenderId(node.getId());
        visible = com != null && visible;
        ...
        if (!rendered) {
            ...
            if( com != null )
              Echo.Render.renderComponentAdd(update, com, tdElement);
            
            if (this.columnCount > 1) {
                for (var c = 0; c < this.columnCount - 1; ++c) {
                    var columnElement = document.createElement("td");

                    com = this._getComponentByRenderId(node.getColumn(c));
                    visible = com != null && visible;
                    if( com != null )
                      Echo.Render.renderComponentAdd(update, com, columnElement);
...
//            img.style.height = "10px";
    ...
    _setRowStyle: function(context) {
        ...
        var com = this._getComponentByRenderId(node.getId());
        var nodeLayout = (com == null ? null : com.render("layoutData"));
        var effect = context.getDefaultEffect();
        var index = -1;
        ...
        while (cellElement) {
            ...
            var columnLayout;
            if (index > -1) {
                com = this._getComponentByRenderId(node.getColumn(index));
                columnLayout = (com == null ? null : com.render("layoutData"));
            } else {
