...
Extras.Sync.Group = Core.extend(Echo.Render.ComponentSync, {
    ...
    _renderBorder: function(update) {
        ...
        var flags = 0;
				...
				var content_position = "absolute"; // we asume that all childs within the border has height != "auto".
				for(var i = 0; i < this.component.getComponentCount(); i++)
					if(this.component.getComponent(i).render("height", "auto") == "auto") // if the child's height == "auto" OR there is no height, we need static position
						content_position = "static";

				if(this.component.parent)
					if(this.component.parent.render("height", "auto") == "auto")
						content_position = "static"; // or if the parents's height == "auto" OR there is no height
        ...
        bottomRightDiv.style.height = borderPixelInsets.bottom + "px";
				bottomRightDiv.style.position = content_position; // absolute || static
				bottomRightDiv.style.bottom = 0;
				bottomRightDiv.style.left = 0;  
        ...
        bottomLeftDiv.appendChild(bottomDiv);
				...
				rightDiv.style.position = content_position; // absolute || static
				rightDiv.style.bottom = bottomRightDiv.style.height;
				rightDiv.style.left = 0;
				rightDiv.style.top = title ? "16px" : "8px";
    ...
    _renderContent: function(update, parentElement) {
        ...
        div.style.height = parentElement.style.height;
