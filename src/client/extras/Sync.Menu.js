...
Extras.Sync.Menu = Core.extend(Echo.Render.ComponentSync, {
    ...
    $virtual: {
        ...
        activate: function() {
            ...
            this.prev_focused = this.client.application.getFocusedComponent();
        ...
        */
        deactivate: function() {
            ...
            this.client.application.setFocusedComponent(this.prev_focused);
     ...
     */
    addMask: function() {
        if (this._maskDeployed) {
            return;
        }
        this._maskDeployed = true;
...
Extras.Sync.ContextMenu = Core.extend(Extras.Sync.Menu, {
    ...
    /** @see Echo.Render.ComponentSync#renderUpdate */
    renderUpdate: function(update) {
        if ( update.isUpdatedPropertySetIn({ modal: true, stateModel: true, model: true, menuVisible : true  }) ) {
            // partial update
            ...
            var reOpenMenu = this._maskDeployed && (modelUpdate != null || stateModelUpdate != null);
            ...
            var menu_visible = update.getUpdatedProperty('menuVisible');
            if( menu_visible )
            {
              if( menu_visible.newValue )
              {
                var bounds = new Core.Web.Measure.Bounds(this.element);
                this._mouseX = bounds.left;
                this._mouseY = bounds.top + bounds.height;
                this.activate();
                this.activateItem(this.menuModel);
              }
              else
